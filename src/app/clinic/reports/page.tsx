...
import { monthlyRevenueData, topDiagnoses, doctorPerformance, busiestHours } from "@/lib/mock-data";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        ...
        <StatCard title="This Month" value={`₹${(latestMonth.revenue / 1000).toFixed(0)}K`} icon={TrendingUp} color="emerald" />
        <StatCard title="Appointments" value={latestMonth.appointments} icon={CalendarCheck} color="accent" />
        <StatCard title="New Patients" value={latestMonth.patients} icon={Users} color="violet" />
      </div>
              ...
              <TableBody>
                {doctorPerformance.map((doc) => (
                  <TableRow key={doc.name}>
                    ...
                    <TableCell>₹{(doc.revenue / 1000).toFixed(0)}K</TableCell>
                    <TableCell>
                      <Badge variant="secondary" className="bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">
                        {doc.rating}
                      </Badge>
                    </TableCell>
                    <TableCell>{doc.completionRate}%</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>

...
- [ ] **Transcription pipeline** (dual-STT strategy):
  - [ ] Live STT (during call): Deepgram or Sarvam AI via Bolna — real-time, <300ms, enables AI to respond
  - [ ] Post-call STT (after call): OpenAI Whisper (self-hosted, free) — higher accuracy transcript for storage
  - [ ] Whisper processes the call recording → generates clean transcript → stores in DB
  - [ ] Support Hindi, Tamil, Telugu, English, and Hinglish via Whisper multilingual model

...
- [ ] Each conversation stored as structured record:
  ...
  - [ ] Full transcript (generated by Whisper post-call)
- [ ] **Transcription pipeline** (STT provider routing by caller number):
  - [ ] Phase 1 (India market): Sarvam AI for ALL calls — live STT + post-call transcript
  - [ ] Phase 2 (Global): Auto-switch STT provider based on caller's phone country code
  - [ ] Provider routing rules:
    - [ ] +91 (India) → Sarvam AI — best for Hindi, Hinglish, Tamil, Telugu, 22 Indian languages
    - [ ] +1 (US/Canada), +44 (UK), +61 (Australia) → Whisper (self-hosted, free)
    - [ ] +971 (UAE), +966 (Saudi) → Whisper — Arabic + English
    - [ ] +65 (Singapore), +60 (Malaysia) → Sarvam AI — Tamil/Hindi diaspora, or Whisper
    - [ ] All other international → Whisper (100+ languages, free)
  - [ ] STT provider config stored in `stt_routing_rules` table — Super Admin can add/modify rules
  - [ ] Override per clinic: clinic can force a specific STT provider regardless of caller number
  - [ ] Fallback: if primary STT provider fails → auto-fallback to Whisper (self-hosted, always available)
  - [ ] Both live STT (during call) and post-call STT (for storage) use the same routed provider
  - [ ] Phase 1 implementation: hardcode Sarvam AI for everything (no routing needed yet)
  - [ ] Phase 2 implementation: add routing layer when first international clinic onboards
...
- [ ] **Embedding generation** from conversations:
  - [ ] After Whisper transcript is ready, generate embedding and store in pgvector
  - [ ] Patient can later ask: "What did the receptionist tell me about my appointment?"
  - [ ] Doctor can later ask: "What did this patient say on the call?"
- [ ] **Conversation visibility** (who can see what):
  - [ ] **Patient portal** → patient sees their own conversation history:
    - [ ] List of all calls, WhatsApp chats, and web chats
    - [ ] Full transcript of each conversation (readable text format)
    - [ ] Actions taken: "Appointment booked with Dr. Sharma, Token #4"
    - [ ] Audio playback of call recording (with consent)
  - [ ] **Doctor/Clinic dashboard** → doctor/staff sees patient's conversations:
    - [ ] All calls and chats for patients in THEIR clinic only (tenant-scoped)
    - [ ] AI-generated summary at top ("Patient called about tooth pain, booked with Dr. Sharma")
    - [ ] Full transcript expandable
    - [ ] Searchable: doctor can search across all conversations for this patient
  - [ ] **AI Chat RAG** → conversations are searchable via AI:
    - [ ] Patient asks AI: "What did the clinic tell me about my appointment?"
    - [ ] Doctor asks AI: "What symptoms did this patient report on the call?"
    - [ ] AI searches conversation embeddings → answers from stored data

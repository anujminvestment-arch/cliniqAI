            ...
            <div className="flex-1 space-y-3">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-muted-foreground">
                    ...
                    <span className="font-semibold text-foreground">{patient.bloodType || patient.blood_type}</span>
                  </p>
                </div>
              ...
              <div className="grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div className="flex items-center gap-2 text-muted-foreground">
                  ...
                  Registered: {patient.registeredDate || patient.registered_date}
                </div>
              </div>
            </div>
          </div>
      ...
      {patient.vitals && patient.vitals.length > 0 && (
        <div>
          ...
          <div className="grid gap-3 grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
            {patient.vitals.map((vital: any, i: number) => {
        ...
        <Card className="animate-fade-in-up delay-300 border-primary/20">
          ...
          <CardContent className="space-y-5">
            <div className="rounded-lg bg-primary/5 p-4 border border-primary/10">
              ...
              <p className="text-base font-medium">{patient.rootCauseAnalysis.primaryDiagnosis || patient.rootCauseAnalysis.primary_diagnosis}</p>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <ul className="space-y-2">
                  {(patient.rootCauseAnalysis.contributingFactors || patient.rootCauseAnalysis.contributing_factors || []).map((factor: string, i: number) => (
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {(patient.rootCauseAnalysis.riskFactors || patient.rootCauseAnalysis.risk_factors || []).map((risk: string, i: number) => (
                    <li key={i} className="flex items-start gap-2 text-sm">
            ...
            <div>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {(patient.rootCauseAnalysis.recommendedInvestigations || patient.rootCauseAnalysis.recommended_investigations || []).map((inv: string, i: number) => (
      ...
      <Tabs defaultValue="transcripts">
        ...
        <TabsContent value="transcripts" className="mt-4 space-y-3">
          ...
          {(!patient.transcripts || patient.transcripts.length === 0) && (
            <div className="text-center py-12 text-muted-foreground">No transcripts available.</div>
          )}
        </TabsContent>
        ...
        <TabsContent value="visits" className="mt-4">
          <Card>
            <CardContent className="p-4">
              {(patient.visitTimeline || patient.visit_timeline || []).length > 0 ? (
                <div className="relative pl-6 space-y-6">
                  <div className="absolute left-[11px] top-2 bottom-2 w-px bg-border" />
                  {(patient.visitTimeline || patient.visit_timeline || []).map((visit: any, i: number) => (
                ...
                <div className="text-center py-12 text-muted-foreground">No visit history available.</div>
              )}
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="prescriptions" className="mt-4 space-y-3">
          {(patient.prescriptionHistory || patient.prescription_history || []).map((rx: any) => (
          ...
          {(!patient.prescriptionHistory && !patient.prescription_history) && (
            <div className="text-center py-12 text-muted-foreground">No prescriptions available.</div>
          )}
        </TabsContent>
        ...
        <TabsContent value="lab" className="mt-4">
          <Card>
            <CardContent className="pt-6">
              {(patient.labResults || patient.lab_results || []).length > 0 ? (
                <Table>
                  <TableHeader>
                    <TableRow>
                  ...
                  <TableBody>
                    {(patient.labResults || patient.lab_results || []).map((lab: any) => (
                      <TableRow key={lab.id}>
                        ...
                        <TableCell className="text-muted-foreground">{lab.normalRange || lab.normal_range}</TableCell>
                        <TableCell>
                          <Badge
                            variant="secondary"
                            className={
                ...
                <div className="text-center py-12 text-muted-foreground">No lab results available.</div>
              )}
            </CardContent>
          </Card>
        </TabsContent>

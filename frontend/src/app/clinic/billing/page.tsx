...
export default function BillingPage() {
  ...
  async function fetchInvoices() {
    try {
      setLoading(true);
      setError(null);
      const res = await billingApi.listInvoices({ limit: PAGE_SIZE, offset: (page - 1) * PAGE_SIZE });
      setInvoicesList(res.invoices || []);
      setTotal(res.total || res.invoices?.length || 0);
    } catch (err: any) {
            ...
            <TableBody>
              {invoicesList.map((invoice) => (
                <TableRow key={invoice.id}>
                  ...
                  <TableCell>{invoice.patient_name || invoice.patient}</TableCell>
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem className="cursor-pointer" onClick={() => alert(`Invoice Details:\n\nID: ${invoice.id}\nPatient: ${invoice.patient_name || invoice.patient}\nAmount: ${(invoice.amount || 0).toLocaleString("en-IN")}\nStatus: ${invoice.status}\nItems: ${invoice.items}`)}>
                          <Eye className="mr-2 h-4 w-4" />
                          View
                        </DropdownMenuItem>
                        <DropdownMenuItem className="cursor-pointer" onClick={() => alert("PDF download will be available soon.")}>
                          <Download className="mr-2 h-4 w-4" />
                          Download PDF
                        </DropdownMenuItem>
                        <DropdownMenuItem className="cursor-pointer" variant="destructive" onClick={() => { if (!window.confirm("Are you sure you want to delete this invoice?")) return; setInvoicesList((prev) => prev.filter((i) => i.id !== invoice.id)); }}>

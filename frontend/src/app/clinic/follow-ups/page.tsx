...
import { followUps as followUpsApi, patients as patientsApi, doctors as doctorsApi, appointments as appointmentsApi } from "@/lib/api";
...
export default function FollowUpsPage() {
  ...
  useEffect(() => {
    Promise.all([
      followUpsApi.list().then((data) => data.follow_ups),
      patientsApi.list().then((data) => data.patients),
      doctorsApi.list().then((data) => data.doctors),
    ])
      .then(([fups, pats, docs]) => {
        setFollowUpsList(fups);
  ...
  async function handleScheduleFollowUp() {
    try {
      await appointmentsApi.create({
        patient_id: fuPatient,
        doctor_id: fuDoctor,
        appointment_date: fuDate,
        type: fuType || "Follow-up",
        notes: fuReason,
      });
      const fups = await followUpsApi.list().then((data) => data.follow_ups);
      setFollowUpsList(fups);
    } catch (err: any) {
      alert(err.message || "Failed to schedule follow-up");
    }
    setDialogOpen(false);
    setFuPatient(""); setFuDoctor(""); setFuDate(""); setFuType(""); setFuReason("");
  }
    ...
    <div className="space-y-6">
      <Breadcrumb className="mb-4">
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem><BreadcrumbPage>Follow-ups</BreadcrumbPage></BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <div className="grid gap-4 py-2">
              <div className="grid gap-2">
                <Label>Patient</Label>
                <Select value={fuPatient} onValueChange={(v) => setFuPatient(v ?? "")}>
              ...
              <div className="grid gap-2">
                <Label>Doctor</Label>
                <Select value={fuDoctor} onValueChange={(v) => setFuDoctor(v ?? "")}>
                  <SelectTrigger className="w-full">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="grid gap-2">
                  <Label>Type</Label>
                  <Select value={fuType} onValueChange={(v) => setFuType(v ?? "")}>
                    <SelectTrigger className="w-full">
              ...
              <div className="grid gap-2">
                <Label>Reason</Label>
                <Input placeholder="e.g. Post-procedure check" value={fuReason} onChange={(e) => setFuReason(e.target.value)} />
              </div>
            </div>
            <DialogFooter>
              ...
              <Button className="cursor-pointer" onClick={handleScheduleFollowUp}>
                Schedule
              </Button>
            </DialogFooter>
          </DialogContent>

...
import { consultations as consultationsApi, doctors as doctorsApi } from "@/lib/api";
...
export default function ConsultationsPage() {
  ...
  useEffect(() => {
    Promise.all([
      consultationsApi.list({ limit: PAGE_SIZE, offset: (page - 1) * PAGE_SIZE }),
      doctorsApi.list().then((data) => data.doctors),
    ])
      .then(([consultsRes, docs]) => {
        setConsultationsList(consultsRes.consultations);
        setTotal(consultsRes.total || consultsRes.consultations?.length || 0);
        setDoctorsList(docs);
      })
      .catch(() => {
  ...
  const avgDuration = totalConsultations > 0
    ? Math.round(
        consultationsList.reduce((sum, c) => sum + parseInt(c.duration || "0"), 0) / totalConsultations
      ) + " min"
    : "0 min";
    ...
    <div className="space-y-6">
      <Breadcrumb className="mb-4">
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem><BreadcrumbPage>Consultations</BreadcrumbPage></BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      <div>
                      ...
                      <div className="flex flex-wrap gap-4 pt-1">
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {(consultation.symptoms || []).map((s: string) => (

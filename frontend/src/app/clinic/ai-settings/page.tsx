...
export default function AISettingsPage() {
  ...
  useEffect(() => {
    analyticsApi.dashboard()
      .then((data: any) => {
        const callAnalytics = data.ai_call_analytics || data.aiCallAnalytics || [];
        if (callAnalytics.length > 0) {
          setLatestStats(callAnalytics[callAnalytics.length - 1]);
        } else if (data.ai_stats) {
          setLatestStats(data.ai_stats);
        }
      })
    ...
    <div className="space-y-6">
      <Breadcrumb className="mb-4">
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem><BreadcrumbPage>AI Settings</BreadcrumbPage></BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      <div>
        ...
        <div>
          <Card>
            <CardHeader>
              ...
              {latestStats?.date && <CardDescription>{latestStats.date}</CardDescription>}
            </CardHeader>
            <CardContent className="space-y-4">
              {latestStats ? (
                <>
                  ...
                  <div className="rounded-lg border p-3 text-center">
                    <p className="text-lg font-bold text-primary">
                      {latestStats.calls > 0 ? Math.round((latestStats.booked / latestStats.calls) * 100) : 0}%
                    </p>
                ...
                <div className="text-center py-8 text-muted-foreground text-sm">
                  No AI call stats available yet.
                </div>
              )}
            </CardContent>
          </Card>

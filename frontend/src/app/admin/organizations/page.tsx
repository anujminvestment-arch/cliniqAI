...
export default function OrganizationsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setOrgGrowthData(Array.isArray(trendsData) ? trendsData : (trendsData as any).orgGrowth || []);
      } catch (err) {
  ...
  const filtered = organizationsList.filter((org: any) => {
    const matchesSearch =
  ...
  const activeOrgs = organizationsList.filter((o: any) => o.status === "active").length;
  const totalClinicsAll = organizationsList.reduce((sum: number, o: any) => sum + (o.totalClinics || 0), 0);
  const totalRevenueAll = organizationsList.reduce((sum: number, o: any) => sum + (o.monthlyRevenue || 0), 0);
    ...
    <div className="space-y-6">
      <Breadcrumb className="mb-4">
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem><BreadcrumbPage>Organizations</BreadcrumbPage></BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList>
          ...
          <TabsTrigger value="active" className="cursor-pointer">
            Active ({organizationsList.filter((o: any) => o.status === "active").length})
          </TabsTrigger>
          <TabsTrigger value="pending" className="cursor-pointer">
            Pending ({organizationsList.filter((o: any) => o.status === "pending").length})
          </TabsTrigger>
          <TabsTrigger value="suspended" className="cursor-pointer">
            Suspended ({organizationsList.filter((o: any) => o.status === "suspended").length})
          </TabsTrigger>
        </TabsList>
                ...
                <TableBody>
                  {filtered.map((org: any) => (
                    <TableRow key={org.id}>
                      ...
                      <TableCell className="text-center">{(org.totalPatients || 0).toLocaleString()}</TableCell>
                      <TableCell className="font-medium">
                        {(org.monthlyRevenue || 0) > 0 ? `₹${((org.monthlyRevenue || 0) / 1000).toFixed(0)}K` : "\u2014"}
                      </TableCell>
                      <TableCell>

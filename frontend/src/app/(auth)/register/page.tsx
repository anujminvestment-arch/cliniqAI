...
function getPasswordStrength(password: string): { score: number; label: string; color: string; width: string } {
  if (password.length < 6) return { score: 0, label: "Too short", color: "bg-red-500", width: "20%" };
  let score = 0;
  ...
  if (score <= 1) return { score, label: "Weak", color: "bg-red-500", width: "40%" };
  if (score === 2) return { score, label: "Fair", color: "bg-amber-500", width: "60%" };
  if (score === 3) return { score, label: "Good", color: "bg-emerald-500", width: "80%" };
...
export default function RegisterPage() {
  ...
  async function handleSubmitForm(e: React.FormEvent) {
    ...
    try {
      ...
      const regData: Record<string, string | undefined> = {
      ...
      if (accountType === "doctor") {
        regData.clinic_name = formData.clinicName;
        regData.clinic_slug = formData.clinicName.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "");
      }
      const data: any = await auth.register(regData);
